...
import { Search, Users, Target, Network, Eye, Plus, X, Trash2, Edit2, Check } from "lucide-react";
...
const NSKAfdeling = ({ currentUser, isAdmin }: NSKAfdelingProps) => {
  const [tab, setTab] = useState<"tavle" | "netvaerk" | "observationer">("tavle");
  ...
  const handleSubmit = () => {
    ...
    if (editId) {
      ...
      const nyt: BandeTilhoer = {
        ...
        personNavn: formNavn, personCpr: formCpr, bande: formBande,
        rolle: formRolle, status: formStatus, noter: formNoter,
        tilfojetAf: userName, tilfojetDato: now,
      };
      save([nyt, ...tilhoer]);
    }
    resetForm();
  ...
  const startEdit = (t: BandeTilhoer) => {
    setEditId(t.id);
    setFormNavn(t.personNavn); setFormCpr(t.personCpr); setFormBande(t.bande);
    setFormRolle(t.rolle); setFormStatus(t.status); setFormNoter(t.noter);
    setShowForm(true);
  };
    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Netværkskort, bandesporing & efterforskning</p>
        </div>
      </div>

      <div className="flex gap-1 mb-4 border-b border-border">
        {[
          { id: "tavle" as const, label: "Opslagstavle", icon: Target },
      ...
      {tab === "tavle" && (
        <AfdelingsIndhold afdelingId="nsk" currentUserNavn={userName} isLeder={isLeder} />
      )}
          ...
          {showForm && (
            <div className="rounded-lg border border-primary/20 bg-card p-4 space-y-3">
              ...
              <div className="flex gap-2 items-center">
                <select
                  value={formStatus}
                  onChange={e => setFormStatus(e.target.value as BandeTilhoer["status"])}
              ...
              <div className="flex gap-2 justify-end">
                ...
                <Button size="sm" className="h-7 text-xs" onClick={handleSubmit}>
                  <Check className="w-3 h-3 mr-1" /> {editId ? "Opdater" : "Tilføj"}
                </Button>
              </div>
            </div>
          )}
                  ...
                  <div key={t.id} className="px-3 py-2 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {t.rolle && (
                          <Badge variant="outline" className="text-[9px] px-1 py-0">{t.rolle}</Badge>
                        )}
                    ...
                    <div className="flex gap-1 shrink-0">
                      <button onClick={() => startEdit(t)} className="p-1 rounded hover:bg-muted transition-colors">
                        <Edit2 className="w-3 h-3 text-muted-foreground" />
                      </button>
                      <button onClick={() => slet(t.id)} className="p-1 rounded hover:bg-muted transition-colors">
